...
%      Version: $Id: gaussmixt.m 5091 2014-09-05 08:20:54Z dmb $
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
persistent r13 r21 r22 r31 r312 r1223 r321 ch1h r122 r124
if isempty(r21)
    r13=[1 3];
    r21=[2 1];
    r22=[2 2];
...
if p==1
    ...
    if ~f1 && ~f2 % both diagonal covariances
        ...
        if p==2                 % special code for 2D vectors
            ...
            vx=px./repmat((px(:,1).*px(:,3)-px(:,2).^2),1,3);
            m=pm1(j1,:)+pm2(j2,:);
            m=m.*vx(:,r13)+m(:,r21).*vx(:,r22);
            v=reshape(vx(:,r1223)',[2 2 k]);
            m12=m1(j1,:)-m2(j2,:);
            dij=vijx(:,1).*vijx(:,3)-vijx(:,2).^2;
            wx=m12(:,[1 1 2]).*m12(:,r122).*vijx(:,r321)*ch1h./dij;
            w=w1(j1).*w2(j2)./sqrt(dij);
        else
            if ~f2  % GMM 2 is diagonal
