function [ stats ] = recallPrecision(computed,actual)
    stats = cell(7,2);
    stats{1,1} = 'TP'; TP = 0; % True Positive (TP) - speech classified correctly
    stats{2,1} = 'TN'; TN = 0; % True Negative (TN) - non-speech classified correctly
    stats{3,1} = 'FP'; FP = 0; % False Positive (FP) - non-speech classified incorrectly
    stats{4,1} = 'FN'; FN = 0; % False Negative (FN) - speech classified incorrectly
